...
module.exports = {
  async find(ctx) {
    ...
    return [...ownMealplaners, ...sharedMealplaners]
      .map((mealplaner) => strapi.config.functions.helpers.mapEntityToIdAndTitle(mealplaner));
  },
  async findOne(ctx){
    const mealplaner = await strapi.services.mealplaner.findOne({id: ctx.params.id});
    return strapi.config.functions.helpers.mapEntityToIdAndTitle(mealplaner);
  },
  async create(ctx) {
